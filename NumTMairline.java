...
public class NumTMairline extends Configured implements Tool {

	public int run(String[] args) throws Exception {

		Job stateWiseCount = Job.getInstance(getConf());
		stateWiseCount.setJobName("Airlines Data Analysis");
		stateWiseCount.setJarByClass(NumTMairline.class);

		/* Field separator for reducer output*/
		stateWiseCount.getConfiguration().set("mapreduce.output.textoutputformat.separator", " | ");
		
		stateWiseCount.setMapperClass(NumTMairlineMapper.class);
		stateWiseCount.setReducerClass(NumTMairlineReducer.class);
		...
		Path outputFilePath = new Path("output33s");
	...
	public static void main(String[] args) throws Exception {
		ToolRunner.run(new NumTMairline(), args);
	}
