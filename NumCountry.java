...
public class NumCountry extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		stateWiseCount.setJarByClass(NumCountry.class);

		/* Field separator for reducer output*/
		stateWiseCount.getConfiguration().set("mapreduce.output.textoutputformat.separator", " | ");
		
		stateWiseCount.setMapperClass(NumCountryMapper.class);
		stateWiseCount.setReducerClass(NumCountryReducer.class);
		...
		Path outputFilePath = new Path("output3s");
	...
	public static void main(String[] args) throws Exception {
		ToolRunner.run(new NumCountry(), args);
	}
